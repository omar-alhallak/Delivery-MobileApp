...
namespace DeliveryApp.API.Controllers
{
    ...
    public sealed class OrdersController : ControllerBase // Controller يفتح endpoints الخاصة بالطلبات للفرونت
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IReadOnlyList<OrderDto>>> GetAll(CancellationToken ct) // منجيب كل الطلبات الموجودة حتى نعرضها بالقائمة
        {
        ...
        [HttpGet("{id:guid}")]
        public async Task<ActionResult<OrderDto>> GetById(Guid id, CancellationToken ct)    // منجيب طلب واحد وتفاصيله عن طريق ال  id تبعه
        {
        ...
        [HttpPost]
        public async Task<ActionResult<OrderDto>> Create([FromBody] CreateOrderRequest request, CancellationToken ct) // الزبون بيأكد طلبه  ومننشئه ومنبعته مباشرة للمطعم
        {
            try
            {
        ...
        [HttpDelete("{id:guid}")]
        public async Task<IActionResult> Delete(Guid id, CancellationToken ct) // منحذف الطلب نهائياً مع تعيينات السائقين المرتبطة فيه
        {
            var deleted = await _deleteOrderService.ExecuteAsync(id, ct);
            return deleted ? NoContent() : NotFound();
        }

        // Endpoint مساعد للطلبات القديمة فقط، ويمكن حذفه لاحقاً لأن POST صار يرسل الطلب مباشرة للمطعم.
        // [HttpPatch("{id:guid}/submit-to-merchant")]
        // public Task<IActionResult> SubmitToMerchant(Guid id, CancellationToken ct)
        //     => RunOrderChange(() => _orderWorkflowService.SubmitToMerchantAsync(id, ct));

        [HttpPatch("{id:guid}/approve-by-merchant")]
        public Task<IActionResult> ApproveByMerchant(Guid id, CancellationToken ct) // المطعم بيقبل الطلب وبيبلش يجهزه
            => RunOrderChange(() => _merchantDecisionService.ApproveAsync(id, ct));

        [HttpPatch("{id:guid}/reject-by-merchant")]
        public Task<IActionResult> RejectByMerchant(Guid id, [FromBody] MerchantDecisionRequest request, CancellationToken ct) // المطعم بيرفض الطلب وبيبعت سبب الرفض
            => RunOrderChange(() => _merchantDecisionService.RejectAsync(id, request, ct));

        [HttpPatch("{id:guid}/ready-for-pickup")]
        public Task<IActionResult> ReadyForPickup(Guid id, CancellationToken ct) // المطعم خلص تجهيز الطلب وصار جاهز ليستلمه السائق
            => RunOrderChange(() => _orderWorkflowService.ReadyForPickupAsync(id, ct));

        [HttpPatch("{id:guid}/picked-up")]
        public Task<IActionResult> PickedUp(Guid id, CancellationToken ct) // المطعم سلّم الطلب للسائق، وعند الزبون بيصير الطلب بالطريق
            => RunOrderChange(() => _orderWorkflowService.PickedUpAsync(id, ct));

        // لم نعد نحتاج هذا الـ Endpoint لأن picked-up ينقل الطلب مباشرة إلى OnTheWay.
        ...
        public Task<IActionResult> Delivered(Guid id, CancellationToken ct) // منعلم الطلب إنو وصل واتسلم للزبون
            => RunOrderChange(() => _orderWorkflowService.DeliveredAsync(id, ct));

        [HttpPatch("{id:guid}/cancel")]
        public Task<IActionResult> Cancel(Guid id, [FromBody] CancelOrderRequest request, CancellationToken ct) // منلغي الطلب ومنسجل مين لغاه وليش
            => RunOrderChange(() => _cancelOrderService.ExecuteAsync(id, request, ct));

        [HttpPatch("{id:guid}/paid")]
        public Task<IActionResult> MarkAsPaid(Guid id, CancellationToken ct) // منعلم الطلب إنو حسابه اندفع
            => RunOrderChange(() => _orderPaymentService.MarkAsPaidAsync(id, ct));

        [HttpPatch("{id:guid}/unpaid")]
        public Task<IActionResult> MarkAsUnpaid(Guid id, CancellationToken ct) // منعلم الطلب إنو حسابه لسا مو مدفوع
